...
print('Beta values of all voxels:', beta)
...
#print('cshape', c)
floor_t = c.dot(npl.pinv(X.T.dot(X))).dot(c)
#print('floor', floor_t)

t = c.T @ beta_2 / np.sqrt(sigma * floor_t)

t_3d = np.zeros(y.shape[:3])
t_3d[mask] = t

t_dist = stats.t(error)
p = 1 - t_dist.cdf(t)
print('P_values:', p)
p_3d = np.zeros(y.shape[:3])
p_3d[mask] = p
